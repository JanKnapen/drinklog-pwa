...
def test_confirm_all_creates_template_for_new_entry(client):
    ...
    r = client.post("/api/alcohol-entries/confirm-all", json={"cutoff": _today_midnight()})
    assert r.status_code == 200
    assert r.json()["confirmed"] == 1

    templates = client.get("/api/alcohol-templates").json()
...
def test_confirm_all_does_not_touch_todays_entries(client):
    ...
    client.post("/api/alcohol-entries", json={"ml": 330, "abv": 5.0, "timestamp": now})
    r = client.post("/api/alcohol-entries/confirm-all", json={"cutoff": _today_midnight()})
    assert r.json()["confirmed"] == 0
    assert client.get("/api/alcohol-entries").json()[0]["is_marked"] is False


def test_confirm_all_enter_ml_confirmed_no_template_created(client):
    client.post("/api/alcohol-entries", json={"ml": 330, "abv": 5.0, "timestamp": _yesterday()})
    client.post("/api/alcohol-entries/confirm-all", json={"cutoff": _today_midnight()})
...
def test_confirm_all_idempotent(client):
    ...
    client.post("/api/alcohol-entries/confirm-all", json={"cutoff": _today_midnight()})
    r2 = client.post("/api/alcohol-entries/confirm-all", json={"cutoff": _today_midnight()})
    assert r2.json()["confirmed"] == 0
    assert len(client.get("/api/alcohol-templates").json()) == 1  # no duplicate template
