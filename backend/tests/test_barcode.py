...
def test_off_plus_volume_from_serving_size(client):
    """Uses serving_size when quantity is absent."""
    payload = {
        "status": 1,
        "product": {
    ...
    mock_client = _mock_off(payload)
    with patch("routers.barcode.httpx.AsyncClient", return_value=mock_client):
        r = client.get("/api/barcode/TESTSERVING?module=alcohol")
    assert r.status_code == 200
    d = r.json()
    assert d["ml"] == 330.0
    assert d["abv"] == 5.0
...
def test_off_plus_abv_from_alcohol_100g(client):
    """Falls back to alcohol_100g when alcohol/alcohol_value absent."""
    payload = {
        "status": 1,
        "product": {
    ...
    mock_client = _mock_off(payload)
    with patch("routers.barcode.httpx.AsyncClient", return_value=mock_client):
        r = client.get("/api/barcode/TESTALC100G?module=alcohol")
    assert r.status_code == 200
    d = r.json()
...
def test_off_plus_caffeine_serving_in_grams(client):
    """caffeine_serving is in grams in OFF; converts correctly to mg."""
    payload = {
        "status": 1,
        "product": {
    ...
    mock_client = _mock_off(payload)
    with patch("routers.barcode.httpx.AsyncClient", return_value=mock_client):
        r = client.get("/api/barcode/TESTCAFS?module=caffeine")
    assert r.status_code == 200
    d = r.json()
    assert d["mg"] == pytest.approx(80.0)
